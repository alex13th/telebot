...
func TestSimpleBotSendRequest(t *testing.T) {
	...
	t.Run("Request URL", func(t *testing.T) {
		if resp.Request.URL.String() != "https://api.telebot.org/bot***Token***/sendMessage" {
			t.Fail()
		}
	})
...
func TestSimplePollerProceedUpdates(t *testing.T) {
	...
	pol := NewBaseLongPoller(tb, UpdateHandlerMock{err: errors.New("Mock error")})
	err := pol.proceedUpdates(context.Background())
